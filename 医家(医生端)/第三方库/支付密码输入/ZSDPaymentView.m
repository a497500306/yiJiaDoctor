...
-(instancetype)init
{
    ...
    if(self)
    {
        ...
        _paymentForm.completeHandle = ^(NSString *inputPwd)
        {
            ...
            {
                //提示密码正确发送网络请求
                [self.delegate mimashuruwangbi:inputPwd];
            }
            else
            {
                //提示密码错误不发送网络请求
                [self.delegate mimashurucuowu];
            }
        };
...
-(void)setIsChongzhi:(BOOL)isChongzhi{
    _paymentForm.chongzhiBtn.alpha = isChongzhi;
    _paymentForm.chongzhiBtn.enabled = isChongzhi;
}
